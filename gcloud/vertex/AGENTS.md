...
* The project uses Google Cloud for containerized training and serving:
  ...
  * To launch the Vertex AI training job, use `bash gcloud/vertex/scripts/submit_vertex_training.sh`.
  * To verify the deployed Cloud Run API, use `bash gcloud/vertex/scripts/verify_cloud_run.sh`.
  * Equivalent invoke commands are available:
    * Build and push GCP images: `uv run invoke cloud-build`.
    * Launch Vertex AI training: `uv run invoke vertex-train`.
    * Build images and launch Vertex AI training: `uv run invoke cloud-train`.
    * List or inspect Vertex AI jobs: `uv run invoke vertex-results`.
